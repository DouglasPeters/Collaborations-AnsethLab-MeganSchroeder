...
Folder = 'F:\DATA\20210308_Bob valves OPN aSMA\SplitTIFFs\'; %This tells the script where to find your images (must keep the trailing slash).
...
Image_Save = 0; %Do you want to save images of the dilation analysis as .jpg files? (Yes=1, No=0)
...
disp('Extracting image information...');
    ...
    for i = 1:ZPlanes
        ...
        if Channels>3
            Ch4.Image(:,:,i) = I{1,1}{((3*ZPlanes)+1),1};
    ...
    if Channels > 2
        Results(f-ff,1).Ch3IntDen = sum(Ch3.Linear(:));
        Results(f-ff,1).Ch3MeanInt = mean(Ch3.Linear(:));    
    else end
    
    if Channels > 3
        Results(f-ff,1).Ch4IntDen = sum(Ch4.Linear(:));
        Results(f-ff,1).Ch4MeanInt = mean(Ch4.Linear(:));     
    else end
...
%     DAPI.BW1 = bwareaopen(DAPI.BW1,50,4);
...
if Image_Show == 1
    ...
    elseif Ch_Show ==2, imshow(Ch2.MedianImage.*10);
    elseif Ch_Show ==3, imshow(Ch3.MedianImage.*10);
    elseif Ch_Show ==4, imshow(Ch4.MedianImage.*10);
    else
